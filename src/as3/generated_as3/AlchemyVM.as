...
// OPTIMIZED?
public function memset (dst:int, v:int, size:int):int
{ 
  var ds:ByteArray = gstate.ds; // eliminate lookup
  var w:int;    // Only set w if necessary
  
  ds.position = dst;
  
  while(size >= 4)
  {
      ds.writeUnsignedInt( w || ( w=v|(v << 8)|(v << 16)|(v << 24) ) ); // Set w the first time it's requested
      size -= 4;
  }
