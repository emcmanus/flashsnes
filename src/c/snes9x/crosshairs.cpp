...
#ifdef __FLASH__
extern void S9xTracef( const char *format, ... );
#endif
...
bool S9xLoadCrosshairFile(int idx, const char *filename){
    ...
    if((s=(char *)calloc(15*15+1, sizeof(char)))==NULL){
        S9xTracef( "STDERR: S9xLoadCrosshairFile: malloc error while reading ");
        perror(filename);
        return false;
    }

    if((fp=fopen(filename, "rb"))==NULL){
        S9xTracef( "STDERR: S9xLoadCrosshairFile: Couldn't open ");
        perror(filename);
        free(s);
        return false;
    }
    ...
    i=fread(s, 1, 8, fp);
    if(i!=8){
        S9xTracef( "STDERR: S9xLoadCrosshairFile: File is too short!\n");
        free(s);
        fclose(fp);
        return false;
    }
    ...
    if(!png_sig_cmp((png_byte *)s, 0, 8)){
        ...
        if(color_type!=PNG_COLOR_TYPE_PALETTE){
            S9xTracef( "STDERR: S9xLoadCrosshairFile: Input PNG is not a palettized image!\n");
        ...
        if(width!=15 || height!=15){
            S9xTracef( "STDERR: S9xLoadCrosshairFile: Expecting a 15x15 PNG\n");
            png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
            free(s);
            fclose(fp);
            return false;
        ...
        if(num_palette!=3 || num_trans!=1){
            S9xTracef( "STDERR: S9xLoadCrosshairFile: Expecting a 3-color PNG with 1 trasnparent color\n");
            png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
            free(s);
            fclose(fp);
            return false;
        ...
        if(transcol<0 || fgcol<0 || bgcol<0){
            S9xTracef( "STDERR: S9xLoadCrosshairFile: PNG must have 3 colors: white (fg), black (bg), and transparent.\n");
            png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
            free(s);
            fclose(fp);
            return false;
        ...
        for(r=0; r<15*15; r+=15){
            ...
            for(i=0; i<15; i++){
                if(row_pointer[i]==transcol) s[r+i]=' ';
                    ...
                    S9xTracef( "STDERR: S9xLoadCrosshairFile: WTF? This was supposed to be a 3-color PNG!\n");
                    png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
                    free(s);
                    fclose(fp);
                    return false;
...
#endif
    {
        ...
        if(getc(fp)!='\n'){
            S9xTracef( "STDERR: S9xLoadCrosshairFile: Invalid file format!");
#ifndef HAVE_LIBPNG
            S9xTracef( "STDERR:  (note: PNG support is not available)");
#endif
            S9xTracef( "STDERR: \n");
            free(s);
            fclose(fp);
