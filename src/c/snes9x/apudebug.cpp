...
{
    S9xTracef( "Print f: Master volume left: %d, right: %d\n",
	    SoundData.master_volume_left, SoundData.master_volume_right);
    S9xTracef( "Print f: Echo: %s %s, Delay: %d Feedback: %d Left: %d Right: %d\n",
    ...
    S9xTracef( "Print f: Noise: %s, Frequency: %d, Pitch mod: %s\n", as_binary (APU.DSP [APU_NON]),
	    env_counter_table [APU.DSP [APU_FLG] & 0x1f],
	    as_binary (SoundData.pitch_mod));
    extern int FilterTaps [8];

    S9xTracef( "Print f: Filter: ");
    for (int i = 0; i < 8; i++)
	S9xTracef( "Print f: %03d, ", FilterTaps [i]);
    S9xTracef( "Print f: \n");
    for (int J = 0; J < 8; J++)
    {
	register Channel *ch = &SoundData.channels[J];

	S9xTracef( "Print f: %d: ", J);
	if (ch->state == SOUND_SILENT)
	{
	    S9xTracef( "Print f: off\n");
	}
	else
	if (!(so.sound_switch & (1 << J)))
	    S9xTracef( "Print f: muted by user using channel on/off toggle\n");
	else
	{
	    int freq = ch->hertz;
		...
		S9xTracef( "Print f: noise, ");
	    }
	    else
		S9xTracef( "Print f: sample %d, ", APU.DSP [APU_SRCN + J * 0x10]);

	    S9xTracef( "Print f: freq: %d", freq);
	    if (J > 0 && (SoundData.pitch_mod & (1 << J)) &&
		ch->type != SOUND_NOISE)
	    {
		S9xTracef( "Print f: (mod), ");
	    }
	    else
		S9xTracef( "Print f: , ");

	    S9xTracef( "Print f: left: %d, right: %d, ",
	    ...
	    S9xTracef( "Print f: %s envx: %d, target: %d, %ld", ch->state > 9 ? "???" : envelope [ch->state],
