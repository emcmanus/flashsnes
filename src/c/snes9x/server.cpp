...
#ifdef NP_DEBUG
        S9xTracef( "Print f: SERVER: Player %d disconnecting @%ld\n", c + 1, S9xGetMilliTime () - START);
#endif
        if (report_error)
        {
    ...
    do
    {
		...
		continue;
            ...
            else
            {
                got = num_bytes;
#ifdef NP_DEBUG
                S9xTracef( "Print f: SERVER: WSAEMSGSIZE, actual bytes %d while receiving data @%d\n", got, S9xGetMilliTime () - START);
#endif
            }
#else
            return (FALSE);
...
static bool8 S9xNPSSendData (int fd, const uint8 *data, int length)
{
	...
	if (sent < 0)
	{
		...
		|| errno == EWOULDBLOCK
            ...
            {
#ifdef NP_DEBUG
                S9xTracef( "Print f: SERVER: EINTR, EAGAIN or EWOULDBLOCK while sending data @%ld\n", S9xGetMilliTime () - START);
#endif
		continue;
            }
	    return (FALSE);
...
void S9xNPProcessClient (int c)
{
    ...
    {
#ifdef NP_DEBUG
        S9xTracef( "Print f: SERVER: Messages lost from '%s', expected %d, got %d\n",
                NPServer.Clients [c].HostName ?
    ...
    switch (header [2] & 0x3f)
    {
        case NP_CLNT_HELLO:
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Got HELLO from client @%ld\n", S9xGetMilliTime () - START);
#endif
		...
		NPServer.ROMName [29] = 0;
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client is playing: %s, Frame Time: %d @%ld\n", data + 4, READ_LONG (data), S9xGetMilliTime () - START);
#endif
            ...
            *ptr++ = c + 1;
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Sending welcome information to client @%ld...\n", S9xGetMilliTime () - START);
#endif
            ...
            delete data;
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Waiting for a response from the client @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client %d loaded requested ROM @%ld...\n", c, S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client %d received ROM image @%ld...\n", c, S9xGetMilliTime () - START);
#endif
            NPServer.Clients [c].SaidHello = TRUE;
            NPServer.Clients [c].Ready = FALSE;
            NPServer.Clients [c].Paused = FALSE;
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client %d waiting for ROM image @%ld...\n", c, S9xGetMilliTime () - START);
#endif
            NPServer.Clients [c].SaidHello = TRUE;
            NPServer.Clients [c].Ready = FALSE;
            NPServer.Clients [c].Paused = FALSE;
        ...
        case NP_CLNT_READY:
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client %d ready @%ld...\n", c, S9xGetMilliTime () - START);
#endif
...
//S9xTracef( "Print f: SERVER: SaidHello = TRUE, SeqNum = %d @%d\n", NPServer.Clients [c].SendSequenceNum, S9xGetMilliTime () - START);
        ...
        case NP_CLNT_PAUSE:
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Client %d Paused: %s @%ld\n", c, (header [2] & 0x80) ? "YES" : "NO", S9xGetMilliTime () - START);
#endif
...
void S9xNPAcceptClient (int Listen, bool8 block)
{
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: attempting to accept new client connection @%ld\n", S9xGetMilliTime () - START);
#endif
    ...
    {
#ifdef NP_DEBUG
        S9xTracef( "Print f: SERVER: Looking up new client's hostname @%ld\n", S9xGetMilliTime () - START);
#endif
	...
	{
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: resolved new client's hostname (%s) @%ld\n", host->h_name, S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: couldn't resolve new client's hostname (%s) @%ld\n", ip ? ip : "Unknown", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: waiting for HELLO message from new client @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Creating socket @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Binding socket to address and port @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Getting socket to listen @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Init complete @%ld\n", S9xGetMilliTime () - START);
#endif
    return (TRUE);
}
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Pause - %s @%ld\n", paused ? "YES" : "NO", S9xGetMilliTime () - START);
#endif
    uint8 pause [7];
    uint8 *ptr = pause;
    *ptr++ = NP_SERV_MAGIC;
...
#ifdef NP_DEBUG
                    S9xTracef( "Print f: SERVER: Sending RESET to all clients @%ld\n", S9xGetMilliTime () - START);
#endif
                    {
                        uint8 reset [7];
                        uint8 *ptr;
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Server thread exiting @%ld\n", S9xGetMilliTime () - START);
#endif
    S9xNPStopServer ();
}
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Starting server on port %d @%ld\n", port, S9xGetMilliTime () - START);
#endif
    p = port;
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Stopping server @%ld\n", S9xGetMilliTime () - START);
#endif
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Sending ROM image to player %d @%ld\n", c + 1, S9xGetMilliTime () - START);
#endif
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Sending freeze file to player %d @%ld\n", c + 1, S9xGetMilliTime () - START);
#endif
...
#if defined(NP_DEBUG) && NP_DEBUG == 2
    S9xTracef( "Print f: SERVER: not paused @%ld\n", S9xGetMilliTime () - START);
#endif
...
void S9xNPSendROMLoadRequest (const char *filename)
{
    ...
    *ptr++ = NP_SERV_LOAD_ROM;
	...
	{
#ifdef NP_DEBUG
            S9xTracef( "Print f: SERVER: Sending load ROM requesting to player %d @%ld\n", i + 1, S9xGetMilliTime () - START);
#endif
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: Sending S-RAM data to player %d @%ld\n", c + 1, S9xGetMilliTime () - START);
#endif
    uint8 sram [7];
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: WaitForEmulationToComplete start @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: SERVER: WaitForEmulationToComplete end @%ld\n", S9xGetMilliTime () - START);
#endif
}
