...
void ResetLogger()
{
	...
	if (!video)
	{
		S9xTracef( "Print f: Opening %s failed. Logging cancelled.\n", buffer);
		return;
	}
...
void VideoLogger(void *pixels, int width, int height, int depth, int bytes_per_line)
{
	...
	if (video)
	{
		...
		{
			S9xTracef( "Print f: -maxframes hit\ndrift:%d\n",drift);
			S9xExit();
		}
	...
	if (Settings.DisplayPressedKeys==1 || keypressscreen)
	{
		...
		if (Settings.DisplayPressedKeys==1)
			S9xTracef( "STDERR: %s %d           \r", buffer, framecounter);
		//if (keypressscreen)
