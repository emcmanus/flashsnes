...
//	SA1.Executing = !SA1.Waiting && SA1.S9xOpcodes;
	...
	break;
    case 0x2203:
//	S9xTracef( "Print f: SA1 reset vector: %04x\n", byte | (Memory.FillRAM [0x2204] << 8));
	break;
    case 0x2204:
//	S9xTracef( "Print f: SA1 reset vector: %04x\n", (byte << 8) | Memory.FillRAM [0x2203]);
    ...
    case 0x2205:
//	S9xTracef( "Print f: SA1 NMI vector: %04x\n", byte | (Memory.FillRAM [0x2206] << 8));
	break;
    case 0x2206:
//	S9xTracef( "Print f: SA1 NMI vector: %04x\n", (byte << 8) | Memory.FillRAM [0x2205]);
    ...
    case 0x2207:
//	S9xTracef( "Print f: SA1 IRQ vector: %04x\n", byte | (Memory.FillRAM [0x2208] << 8));
	break;
    case 0x2208:
...
//	    SA1.Executing = !SA1.Waiting;
	...
	break;
    case 0x220c:
//	S9xTracef( "Print f: SNES NMI vector: %04x\n", byte | (Memory.FillRAM [0x220d] << 8));
	break;
    case 0x220d:
//	S9xTracef( "Print f: SNES NMI vector: %04x\n", (byte << 8) | Memory.FillRAM [0x220c]);
    ...
    case 0x220e:
//	S9xTracef( "Print f: SNES IRQ vector: %04x\n", byte | (Memory.FillRAM [0x220f] << 8));
	break;
    case 0x220f:
//	S9xTracef( "Print f: SNES IRQ vector: %04x\n", (byte << 8) | Memory.FillRAM [0x220e]);
    ...
    case 0x2210:
#if 0
	S9xTracef( "Print f: Timer %s\n", (byte & 0x80) ? "linear" : "HV");
	S9xTracef( "Print f: Timer H-IRQ %s\n", (byte & 1) ? "enabled" : "disabled");
	S9xTracef( "Print f: Timer V-IRQ %s\n", (byte & 2) ? "enabled" : "disabled");
#endif
	break;
    case 0x2211:
//	S9xTracef( "Print f: Timer reset\n");
	break;
    case 0x2212:
//	S9xTracef( "Print f: H-Timer %04x\n", byte | (Memory.FillRAM [0x2213] << 8));
	break;
    case 0x2213:
//	S9xTracef( "Print f: H-Timer %04x\n", (byte << 8) | Memory.FillRAM [0x2212]);
	break;
    case 0x2214:
//	S9xTracef( "Print f: V-Timer %04x\n", byte | (Memory.FillRAM [0x2215] << 8));
	break;
    case 0x2215:
...
//	S9xTracef( "Print f: MMC: %02x\n", byte);
	break;
    case 0x2224:
//	S9xTracef( "Print f: BWRAM image SNES %02x -> 0x6000\n", byte);
	Memory.BWRAM = Memory.SRAM + (byte & 7) * 0x2000;
	break;
    case 0x2225:
//	S9xTracef( "Print f: BWRAM image SA1 %02x -> 0x6000 (%02x)\n", byte, Memory.FillRAM [address]);
	if (byte != Memory.FillRAM [address])
	    S9xSA1SetBWRAMMemMap (byte);
	break;
    case 0x2226:
//	S9xTracef( "Print f: BW-RAM SNES write %s\n", (byte & 0x80) ? "enabled" : "disabled");
	break;
    case 0x2227:
//	S9xTracef( "Print f: BW-RAM SA1 write %s\n", (byte & 0x80) ? "enabled" : "disabled");
	break;

    case 0x2228:
//	S9xTracef( "Print f: BW-RAM write protect area %02x\n", byte);
	break;
    case 0x2229:
//	S9xTracef( "Print f: I-RAM SNES write protect area %02x\n", byte);
	break;
    case 0x222a:
//	S9xTracef( "Print f: I-RAM SA1 write protect area %02x\n", byte);
	break;
    case 0x2230:
#if 0
	S9xTracef( "Print f: SA1 DMA %s\n", (byte & 0x80) ? "enabled" : "disabled");
	S9xTracef( "Print f: DMA priority %s\n", (byte & 0x40) ? "DMA" : "SA1");
	S9xTracef( "Print f: DMA %s\n", (byte & 0x20) ? "char conv" : "normal");
	S9xTracef( "Print f: DMA type %s\n", (byte & 0x10) ? "BW-RAM -> I-RAM" : "SA1 -> I-RAM");
	S9xTracef( "Print f: DMA distination %s\n", (byte & 4) ? "BW-RAM" : "I-RAM");
	S9xTracef( "Print f: DMA source %s\n", DMAsource [byte & 3]);
#endif
	break;
    case 0x2231:
	if (byte & 0x80)
...
#if 0
	S9xTracef( "Print f: CHDEND %s\n", (byte & 0x80) ? "complete" : "incomplete");
	S9xTracef( "Print f: DMA colour mode %d\n", byte & 3);
	S9xTracef( "Print f: virtual VRAM width %d\n", (byte >> 2) & 7);
#endif
	break;
    case 0x2232:
    case 0x2233:
...
#if 0
	S9xTracef( "Print f: DMA source start %06x\n",
...
#if 0
	S9xTracef( "Print f: DMA dest address %06x\n",
