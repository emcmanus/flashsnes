...
bool ConfigFile::LoadFile(const char *filename){
    ...
    if((s=OPEN_STREAM(filename, "r"))){
        ...
        S9xTracef( "STDERR: Couldn't open conffile ");
        perror(filename);
    }
    return ret;
}
...
void ConfigFile::LoadFile(Reader *r, const char *name){
    ...
    do {
        ...
        if(l[0]=='['){
            if(*l.rbegin()!=']'){
                if(name) S9xTracef( "STDERR: %s:", name);
                S9xTracef( "STDERR: [%d]: Ignoring invalid section header\n", line);
                continue;
            }
        ...
        while(*l.rbegin()=='\\'){
            ...
            val=r->getline(eof);
            if(eof){
                S9xTracef( "STDERR: Unexpected EOF reading config file");
                if(name) S9xTracef( "STDERR:  '%s'", name);
                S9xTracef( "STDERR: \n");
                return;
            }
        ...
        if(i<0){
            if(name) S9xTracef( "STDERR: %s:", name);
            S9xTracef( "STDERR: [%d]: Ignoring invalid entry\n", line);
            continue;
        }
...
bool ConfigFile::SaveTo(const char *filename){
    ...
    if((fp=fopen(filename, "w"))==NULL){
        S9xTracef( "STDERR: Couldn't write conffile ");
        perror(filename);
        return false;
    }
