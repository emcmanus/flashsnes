...
{
//    S9xTracef( "STDERR: %03d: %02x to %04x\n", CPU.V_Counter, Byte, Address);
...
//		CPU.WaitAddress = CPU.PCAtOpcodeStart;
	...
	if (Address <= 0x2fff || Address >= 0x3000 + 768)
	{
	    ...
	    case 0x21c2:
			...
			//	S9xTracef( "STDERR: Read from $21c2!\n");
				return OpenBus;
	    case 0x21c3:
				if(Model->_5C77 ==2)
			        return (0);
			//	S9xTracef( "STDERR: Read from $21c3!\n");
				return OpenBus;
	    case 0x2800:
...
//    S9xTracef( "STDERR: %03d: %02x from %04x\n", CPU.V_Counter, byte, Address);
    return (byte);
}
	...
	if (Address < 0x4200)
	{
		...
		switch (Address)
		{
		  ...
		  case 0x420C:
			...
			//S9xTracef( "Print f: $%02x is written to $420c at HC:%d, V:%d, IPPU.HDMA:$%02x, IPPU.HDMAEnded:$%02x\n", byte, CPU.Cycles, CPU.V_Counter, IPPU.HDMA, IPPU.HDMAEnded);
			// FIXME
		  ...
		  case 0x437A:
			...
			//S9xTracef( "Print f: $%02x is written to $43%da at HC:%d, V:%d, IPPU.HDMA:$%02x, IPPU.HDMAEnded:$%02x\n", byte, d, CPU.Cycles, CPU.V_Counter, IPPU.HDMA, IPPU.HDMAEnded);
			return;
...
uint8 S9xGetCPU (uint16 Address)
{
    int d;
    uint8 byte;
//    S9xTracef( "STDERR: read from %04x\n", Address);
...
void S9xSuperFXExec ()
{
	...
	*(uint16 *) &SRAM [0x002c] = 0x8000;
	...
	while (Memory.FillRAM [0x3030] & 0x20)
	{
		...
		if(((r1+a)&0xffff) != r2)
		    S9xTracef( "Print f:  r%d=$%04x",i,r2);
	    }
	    {
		/* Check SFR */
		...
		if((r1&(1<<1)) != (r2&(1<<1)))
		    S9xTracef( "Print f:  Z=%d",(uint32)(!!(r2&(1<<1))));
		if((r1&(1<<2)) != (r2&(1<<2)))
		    S9xTracef( "Print f:  CY=%d",(uint32)(!!(r2&(1<<2))));
		if((r1&(1<<3)) != (r2&(1<<3)))
		    S9xTracef( "Print f:  S=%d",(uint32)(!!(r2&(1<<3))));
		if((r1&(1<<4)) != (r2&(1<<4)))
		    S9xTracef( "Print f:  OV=%d",(uint32)(!!(r2&(1<<4))));
		if((r1&(1<<5)) != (r2&(1<<5)))
		    S9xTracef( "Print f:  G=%d",(uint32)(!!(r2&(1<<5))));
		if((r1&(1<<6)) != (r2&(1<<6)))
		    S9xTracef( "Print f:  R=%d",(uint32)(!!(r2&(1<<6))));
		if((r1&(1<<8)) != (r2&(1<<8)))
		    S9xTracef( "Print f:  ALT1=%d",(uint32)(!!(r2&(1<<8))));
		if((r1&(1<<9)) != (r2&(1<<9)))
		    S9xTracef( "Print f:  ALT2=%d",(uint32)(!!(r2&(1<<9))));
		if((r1&(1<<10)) != (r2&(1<<10)))
		    S9xTracef( "Print f:  IL=%d",(uint32)(!!(r2&(1<<10))));
		if((r1&(1<<11)) != (r2&(1<<11)))
		    S9xTracef( "Print f:  IH=%d",(uint32)(!!(r2&(1<<11))));
		if((r1&(1<<12)) != (r2&(1<<12)))
		    S9xTracef( "Print f:  B=%d",(uint32)(!!(r2&(1<<12))));
		if((r1&(1<<15)) != (r2&(1<<15)))
		    S9xTracef( "Print f:  IRQ=%d",(uint32)(!!(r2&(1<<15))));
	    }
	    {
		/* Check PBR */
		...
		if(r1 != r2)
		    S9xTracef( "Print f:  ROMBR=$%02x",r2);
	    }
	    {
		/* Check RAMBR */
		...
		if(r1 != r2)
		    S9xTracef( "Print f:  RAMBR=$%02x",r2);
	    }
	    {
		/* Check CBR */
		...
		if(r1 != r2)
		    S9xTracef( "Print f:  CBR=$%04x",r2);
	    }
	    {
		/* Check COLR */
		if(vColr != FxGetColorRegister())
		    S9xTracef( "Print f:  COLR=$%02x",FxGetColorRegister());
	    }
	    {
		/* Check POR */
		if(vPor != FxGetPlotOptionRegister())
		    S9xTracef( "Print f:  POR=$%02x",FxGetPlotOptionRegister());
	    }
	    S9xTracef( "Print f: \n");
	}
	S9xExit ();
