...
void CMemory::ParseSNESHeader (uint8 *RomHeader)
{
	...
	if (bs)
	{
		if (!(((RomHeader[0x29] & 0x20) && CalculatedSize <  0x100000) ||
			...
			S9xTracef( "Print f: BS: Size mismatch\n");
	...
	S9xTracef( "ROMChecksum 1 : " );
	S9xTraceInt( ROMChecksum );
	
	S9xTracef( "ROMComplementChecksum 1 : " );
	S9xTraceInt( ROMComplementChecksum );
...
void CMemory::InitROM (void)
{
	...
	S9xTracef( "CalculatedChecksum 2 : " );
	S9xTraceInt( CalculatedChecksum );
	
	S9xTracef( "ROMChecksum 2 : " );
	S9xTraceInt( ROMChecksum );
	
	S9xTracef( "ROMComplementChecksum 2 : " );
	S9xTraceInt( ROMComplementChecksum );
	
	
	S9xTracef("ROMChecksum + ROMComplementChecksum: ");
	S9xTraceInt( ROMChecksum + ROMComplementChecksum );
	
	S9xTracef("ROMChecksum + ROMComplementChecksum == 0xffff: ");
	S9xTraceInt( (int) ((ROMChecksum + ROMComplementChecksum) == 0xffff)  );
	
	S9xTracef( "ROMChecksum == CalculatedChecksum: " );
	S9xTraceInt( (int) (ROMChecksum == CalculatedChecksum) );
	
	S9xTracef("isChecksumOK = ");
	S9xTraceInt( (int) isChecksumOK );
...
{
	S9xTracef( "Print f: Map_LoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_NoMAD1LoROMMap\n");
	map_System();
...
void CMemory::Map_JumboLoROMMap (void)
{
	...
	S9xTracef( "Print f: Map_JumboLoROMMap\n");
	map_System();
...
void CMemory::Map_ROM24MBSLoROMMap (void)
{
	...
	S9xTracef( "Print f: Map_ROM24MBSLoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SRAM512KLoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SufamiTurboLoROMMap\n");
	map_System();
...
{
	// for combined images
	S9xTracef( "Print f: Map_SufamiTurboPseudoLoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SuperFXLoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SetaDSPLoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SDD1LoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SA1LoROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_HiROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_ExtendedHiROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SameGameHiROMMap\n");
	map_System();
...
{
	S9xTracef( "Print f: Map_SPC7110HiROMMap\n");
	map_System();
...
// Test for Alchemy port
uint16 CMemory::return_test_val ()
{
	uint16  sum = 0;
...
void CMemory::Checksum_Calculate (void)
{
	...
	CalculatedChecksum = (int) CalculatedChecksum;	// this is really bizarre and kinda scary -- Alchemy overflows this value unless cast
	
//	S9xTracef( "Calculated checksum as it's assigned in Checksum_Calculate" );
//	S9xTraceInt( CalculatedChecksum );
}
...
void CMemory::ApplyROMFixes (void)
{
	...
	if (match_id("AVCJ"))                                      // Rendering Ranger R2
	{
		...
		S9xTracef( "Print f: APU OneCycle hack: %d\n", IAPU.OneCycle);
	}
	...
	if (match_na("GUNDAMW ENDLESSDUEL")) // Shin Kidou Senki Gundam W - Endless Duel
	{
		...
		S9xTracef( "Print f: HDMA timing hack: %d\n", Timings.HDMAStart);
	}
		...
		match_nc("MADDEN"))
	{
		if (Settings.Shutdown)
			S9xTracef( "Print f: Disabled CPU shutdown hack.\n");
	...
	if (match_nn("UNIRACERS")) // Uniracers
	{
		...
		S9xTracef( "Print f: Applied Uniracers hack.\n");
	}
    ...
    while(port==UNZ_OK){
        ...
        if(len>=l+1 && name[len-l-1]=='.' && strcasecmp(name+len-l, ext)==0 &&
           unzOpenCurrentFile(file)==UNZ_OK){
            if(print) S9xTracef( "Print f: Using IPS patch %s", name);
            return port;
        }
        port = unzGoToNextFile(file);
    }
...
void CMemory::CheckForIPSPatch (const char *rom_filename, bool8 header, int32 &rom_size)
{
    ...
    if (strcasecmp(ext, "zip")==0) {
        ...
        if(file!=NULL){
            ...
            while(port==UNZ_OK){
                S9xTracef( "Print f:  in %s", rom_filename);
            ...
            if(!flag){
                ...
                do {
                    ...
                    if(unzFindExtension(file, ips, false, false)==UNZ_OK) S9xTracef( "Print f: WARNING: Ignoring extra .%s files!\n", ips);
                } while(++i!=0);
            }
            if(!flag){
                i=0;
    ...
    if((patch_file=OPEN_STREAM(n, "rb"))) {
        S9xTracef( "Print f: Using IPS patch %s", n);
        ret=ReadIPSPatch(new fReader(patch_file), offset, rom_size);
        CLOSE_STREAM(patch_file);
        if(ret){
            S9xTracef( "Print f: !\n");
