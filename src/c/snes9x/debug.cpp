...
static void ProcessDebugCommand (char *Line)
{
	...
	S9xTracef( "Print f: Cheat Search Started\n");
	return;
    }
    if (strcasecmp (Line, "less") == 0)
    {
	S9xSearchForChange (&Cheat, S9X_LESS_THAN, S9X_8_BITS, FALSE, TRUE);
	S9xTracef( "Print f: Recorded all values that have decreased\n");
	return;
    }
    if (strcasecmp (Line, "print") == 0)
    {
	S9xTracef( "Print f: Cheat search results:\n");
	...
	if (sscanf (&Line [4], "%x %d", &Address, &Count) == 2)
	{
		...
		S9xTracef( "Print f: Can't open %s for writing\n", String);
	}
	else
	    S9xTracef( "Print f: Usage: dump start_address_in_hex count_in_decimal\n");
	return;
    }
    if (Line[0] == 'i')
    {
	S9xTracef( "Print f: Vectors:\n");
    ...
    {
	S9xTracef( "Print f: APU vectors:");
	for (int i = 0; i < 0x40; i += 2)
	{
	    if (i % 16 == 0)
		S9xTracef( "Print f: \n%04x ", 0xffc0 + i);
	    S9xTracef( "Print f: %04x ", APU.ExtraRAM [i]);
	}
	S9xTracef( "Print f: \n");
    }
    if (Line[0] == 's')
    ...
    if (*Line == 'c')
    {
	S9xTracef( "Print f: Colours:\n");
	for (int i = 0; i < 256; i++)
	{
	    S9xTracef( "Print f: %02x%02x%02x  ", PPU.CGDATA[i] & 0x1f,
		    (PPU.CGDATA[i] >> 5) & 0x1f,
	...
	S9xTracef( "Print f: Sprites: Small: %dx%d, Large: %dx%d, OAMAddr: 0x%04x, OBJNameBase: 0x%04x, OBJNameSelect: 0x%04x, First: %d\n",
...
//		    c++;
//	    }
//	    S9xTracef( "Print f: Priority %d: %03d, ", p, c);
//	}
//	S9xTracef( "Print f: \n");
	for (int i = 0; i < 128; i++)
	{
	    S9xTracef( "Print f: X:%3d Y:%3d %c%c%d%c ",
		    PPU.OBJ[i].HPos,
		    PPU.OBJ[i].VPos,
		    PPU.OBJ[i].VFlip ? 'V' : 'v',
		    PPU.OBJ[i].HFlip ? 'H' : 'h',
	...
	if (Line [1] == 'S')
	{
	    ...
	    {
		S9xTracef( "Print f: SA1 CPU instruction tracing enabled.\n");
		if (trace2 == NULL)
		    trace2 = fopen ("trace_sa1.log", "wb");
	    }
	    else
	    {
		S9xTracef( "Print f: SA1 CPU instruction tracing disabled.\n");
		fclose (trace2);
		trace2 = NULL;
	    }
	}
	else
	{
	    ...
	    {
		S9xTracef( "Print f: CPU instruction tracing enabled.\n");
		if (trace == NULL)
		    trace = fopen ("trace.log", "wb");
	    }
	    else
	    {
		S9xTracef( "Print f: CPU instruction tracing disabled.\n");
		fclose (trace);
		trace = NULL;
	    }
	}
	...
	S9xTracef( "Print f: APU tracing %s\n", APU.Flags & TRACE_FLAG ? "enabled" :
		"disabled");
    }
    if (*Line == 'B')
    {
	Settings.TraceSoundDSP ^= 1;
	S9xTracef( "Print f: Sound DSP register tracing %s\n", Settings.TraceSoundDSP ?
						   "enabled" : "disabled");
    ...
    if (*Line == 'C')
    {
	S9xTracef( "Print f: SPC700 sample addresses at 0x%04x:\n", APU.DSP [APU_DIR] << 8);
	for (int i = 0; i < 256; i++)
	{
	    uint8 *dir = IAPU.RAM +
		    (((APU.DSP [APU_DIR] << 8) +
	    ...
	    S9xTracef( "Print f: %04X %04X;", addr, addr2);
	    if (i % 8 == 7)
		S9xTracef( "Print f: \n");
	}
    }
	...
	S9xTracef( "Print f: APU RAM dump:\n");
	for (uint32 l = 0; l < Count; l += 16)
	{
	    S9xTracef( "Print f: %04X ", Address);
	    for (int i = 0; i < 16; i++)
		S9xTracef( "Print f: %02X ", IAPU.RAM [Address++]);
	    S9xTracef( "Print f: \n");
	}
	*Line = 0;
    }
...
static void PrintWindow (uint8 * a)
{
	...
	if (a[i])
	    ...
	    {
	    case 0:
		S9xTracef( "Print f: Background 0, ");
		break;
	    case 1:
		S9xTracef( "Print f: Background 1, ");
		break;
	    case 2:
		S9xTracef( "Print f: Background 2, ");
		break;
	    case 3:
		S9xTracef( "Print f: Background 3, ");
		break;
	    case 4:
		S9xTracef( "Print f: Objects, ");
		break;
	    case 5:
		S9xTracef( "Print f: Colour window, ");
		break;
	    }
}
...
static void WhatsUsed ()
{
    S9xTracef( "Print f: V-line: %ld, H-Pos: %ld\n", CPU.V_Counter, CPU.Cycles);
    S9xTracef( "Print f: Screen mode: %d, ", PPU.BGMode);
    if (PPU.BGMode <= 1 && (Memory.FillRAM [0x2105] & 8))
	S9xTracef( "Print f: (BG#2 Priority)");

    S9xTracef( "Print f: Brightness: %d", PPU.Brightness);
    if (Memory.FillRAM[0x2100] & 0x80)
	S9xTracef( "Print f:  (screen blanked)");
    S9xTracef( "Print f: \n");
    if (Memory.FillRAM[0x2133] & 1)
	S9xTracef( "Print f: Interlace, ");
    if (Memory.FillRAM[0x2133] & 4)
	S9xTracef( "Print f: 240 line visible, ");
    if (Memory.FillRAM[0x2133] & 8)
	S9xTracef( "Print f: Pseudo 512 pixels horizontal resolution, ");
    if (Memory.FillRAM[0x2133] & 0x40)
	S9xTracef( "Print f: Mode 7 priority per pixel, ");
    S9xTracef( "Print f: \n");
    if (PPU.BGMode == 7 && (Memory.FillRAM[0x211a] & 3))
	S9xTracef( "Print f: Mode 7 flipping, ");
    if (PPU.BGMode == 7)
	S9xTracef( "Print f: Mode 7 screen repeat: %d,", (Memory.FillRAM[0x211a] & 0xc0) >> 6);
    if (Memory.FillRAM[0x2130] & 1)
	S9xTracef( "Print f: 32K colour mode, ");
    if (PPU.BGMode == 7)
    {
	...
	for (int i = 0; i < 4; i++)
		...
		S9xTracef( "Print f: BG%d,", i);
	S9xTracef( "Print f: ,");
    }
    if (PPU.HVBeamCounterLatched)
	S9xTracef( "Print f: V and H beam pos latched, ");
    if (Memory.FillRAM[0x4200] & 0x20)
	S9xTracef( "Print f: V-IRQ enabled at %d\n", PPU.IRQVBeamPos);
    if (Memory.FillRAM[0x4200] & 0x10)
	S9xTracef( "Print f: H-IRQ enabled at %d\n", PPU.IRQHBeamPos);
    if (Memory.FillRAM[0x4200] & 0x80)
	S9xTracef( "Print f: V-blank NMI enabled\n");
    int i;
    for (i = 0; i < 8; i++)
    {
	if (missing.hdma_this_frame & (1 << i))
	{
	    S9xTracef( "Print f: H-DMA %d [%d] 0x%02X%04X->0x21%02X %s %s 0x%02X%04X %s addressing\n",
	...
	{
	    S9xTracef( "Print f: DMA %d %d 0x%02X%04X->0x21%02X Num: %d %s\n",
    ...
    S9xTracef( "Print f: VRAM write address: 0x%04x(%s), Full Graphic: %d, Address inc: %d\n",
	    PPU.VMA.Address,
    ...
    for (i = 0; i < 4; i++)
    {
	S9xTracef( "Print f: BG%d: VOffset:%d, HOffset:%d, W:%d, H:%d, TS:%d, BA:0x%04x, TA:0x%04X\n",
    ...
    S9xTracef( "Print f: Main screen (%s): ", s);
    for (i = 0; i < 5; i++)
	if (Memory.FillRAM[0x212c] & (1 << i))
	    switch (i)
	    {
	    case 0:
		S9xTracef( "Print f: BG0,");
		break;
	    case 1:
		S9xTracef( "Print f: BG1,");
		break;
	    case 2:
		S9xTracef( "Print f: BG2,");
		break;
	    case 3:
		S9xTracef( "Print f: BG3,");
		break;
	    case 4:
		S9xTracef( "Print f: OBJ,");
		break;
	    }
	...
	if (Memory.FillRAM[0x2131] & 0x80)
	{
		...
		S9xTracef( "Print f: Subscreen subtract");
	    else
		S9xTracef( "Print f: Fixed colour subtract");
	}
	else
	{
	    if (Memory.FillRAM[0x2130] & 0x02)
		S9xTracef( "Print f: Subscreen addition");
	    else
		S9xTracef( "Print f: Fixed colour addition");
	}
	if (Memory.FillRAM [0x2131] & 0x40)
	    S9xTracef( "Print f: (half):");
	else
	    S9xTracef( "Print f: :");
	...
	if (PPU.ClipWindow1Enable [i])
	    ...
	    {
	    case 0:
		S9xTracef( "Print f: BG0(%s-%s),", PPU.ClipWindow1Inside [i] ? "I" : "O",
			ClipFn (PPU.ClipWindowOverlapLogic[0]));
		break;
	    case 1:
		S9xTracef( "Print f: BG1(%s-%s),", PPU.ClipWindow1Inside [i] ? "I" : "O",
			ClipFn (PPU.ClipWindowOverlapLogic[1]));
		break;
	    case 2:
		S9xTracef( "Print f: BG2(%s-%s),", PPU.ClipWindow1Inside [i] ? "I" : "O",
			ClipFn (PPU.ClipWindowOverlapLogic[2]));
		break;
	    case 3:
		S9xTracef( "Print f: BG3(%s-%s),", PPU.ClipWindow1Inside [i] ? "I" : "O",
			ClipFn (PPU.ClipWindowOverlapLogic[3]));
		break;
	    case 4:
		S9xTracef( "Print f: OBJ(%s-%s),", PPU.ClipWindow1Inside [i] ? "I" : "O",
			ClipFn (PPU.ClipWindowOverlapLogic[4]));
		break;
	    case 5:
		S9xTracef( "Print f: COL(%s-%s)", PPU.ClipWindow1Inside [i] ? "I" : "O",
	...
	if (PPU.ClipWindow2Enable [i])
	    ...
	    {
	    case 0:
		S9xTracef( "Print f: BG0(%s),", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    case 1:
		S9xTracef( "Print f: BG1(%s),", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    case 2:
		S9xTracef( "Print f: BG2(%s),", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    case 3:
		S9xTracef( "Print f: BG3(%s),", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    case 4:
		S9xTracef( "Print f: OBJ(%s),", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    case 5:
		S9xTracef( "Print f: COL(%s)", PPU.ClipWindow2Inside [i] ? "I" : "O");
		break;
	    }
...
static void WhatsMissing ()
{
    S9xTracef( "Print f: Processor: ");
    if (missing.emulate6502)
	S9xTracef( "Print f: emulation mode, ");
    if (missing.decimal_mode)
	S9xTracef( "Print f: decimal mode,");
    if (missing.mv_8bit_index)
	S9xTracef( "Print f: MVP/MVN with 8bit index registers and XH or YH > 0,");
    if (missing.mv_8bit_acc)
	S9xTracef( "Print f: MVP/MVN with 8bit accumulator > 255");
    S9xTracef( "Print f: \nScreen modes used:");
    int i;
    for (i = 0; i < 8; i++)
	if (missing.modes[i])
	    S9xTracef( "Print f:  %d,", i);
    S9xTracef( "Print f: \n");
	...
	S9xTracef( "Print f: 240 lines visible,");
    if (missing.sprite_double_height)
	S9xTracef( "Print f: double-hight sprites,");
    S9xTracef( "Print f: \n");
    if (missing.mode7_fx)
	S9xTracef( "Print f: Mode 7 rotation/scaling, ");
    if (missing.matrix_read)
	S9xTracef( "Print f: Mode 7 read matrix registers, ");
    if (missing.mode7_flip)
	S9xTracef( "Print f: Mode 7 flipping, ");
    if (missing.mode7_bgmode)
	S9xTracef( "Print f: Mode 7 priority per pixel, ");
    if (missing.direct)
	S9xTracef( "Print f: Direct 32000 colour mode");
    S9xTracef( "Print f: \n");
    if (missing.mosaic)
	S9xTracef( "Print f: Mosaic effect, ");
    if (missing.subscreen)
	S9xTracef( "Print f: Subscreen enabled, ");
    if (missing.subscreen_add)
	S9xTracef( "Print f: Subscreen colour add, ");
    if (missing.subscreen_sub)
	S9xTracef( "Print f: Subscreen colour subtract, ");
    if (missing.fixed_colour_add)
	S9xTracef( "Print f: Fixed colour add, ");
    if (missing.fixed_colour_sub)
	S9xTracef( "Print f: Fixed colour subtract");
    S9xTracef( "Print f: \n");
    S9xTracef( "Print f: Window 1 enabled on:");
    PrintWindow (missing.window1);
    S9xTracef( "Print f: \nWindow 2 enabled on:");
    PrintWindow (missing.window2);
    S9xTracef( "Print f: \n");
    if (missing.bg_offset_read)
	S9xTracef( "Print f: BG offset read, ");
    if (missing.oam_address_read)
	S9xTracef( "Print f: OAM address read,");
    if (missing.sprite_priority_rotation)
	S9xTracef( "Print f: Sprite priority rotation, ");
    if (missing.fast_rom)
	S9xTracef( "Print f: Fast 3.58MHz ROM access enabled, ");
    if (missing.matrix_multiply)
	S9xTracef( "Print f: Matrix multiply 16bit by 8bit used");
    S9xTracef( "Print f: \n");
    if (missing.virq)
	S9xTracef( "Print f: V-position IRQ used at line %d, ", missing.virq_pos);
    if (missing.hirq)
	S9xTracef( "Print f: H-position IRQ used at position %d, ", missing.hirq_pos);
    S9xTracef( "Print f: \n");
    if (missing.h_v_latch)
	S9xTracef( "Print f: H and V-Pos latched, ");
    if (missing.h_counter_read)
	S9xTracef( "Print f: H-Pos read, ");
    if (missing.v_counter_read)
	S9xTracef( "Print f: V-Pos read");
    S9xTracef( "Print f: \n");
    if (missing.oam_read)
	S9xTracef( "Print f: OAM read, ");
    if (missing.vram_read)
	S9xTracef( "Print f: VRAM read, ");
    if (missing.cgram_read)
	S9xTracef( "Print f: CG-RAM read, ");
    if (missing.wram_read)
	S9xTracef( "Print f: WRAM read, ");
    if (missing.dma_read)
	S9xTracef( "Print f: DMA read,");
    if (missing.vram_inc)
	S9xTracef( "Print f: VRAM inc: %d,", missing.vram_inc);
    if (missing.vram_full_graphic_inc)
	S9xTracef( "Print f: VRAM full graphic inc: %d,", missing.vram_full_graphic_inc);
    S9xTracef( "Print f: \n");
    for (i = 0; i < 8; i++)
    {
	if (missing.hdma[i].used)
	{
	    S9xTracef( "Print f: HDMA %d, 0x%02X%04X->0x21%02X %s ", i,
		...
		S9xTracef( "Print f: Forced address write, ");
	    if (missing.hdma[i].force_table_address_read)
		S9xTracef( "Print f: Current address read, ");
	    if (missing.hdma[i].line_count_write)
		S9xTracef( "Print f: Line count write, ");
	    if (missing.hdma[i].line_count_read)
		S9xTracef( "Print f: Line count read");
	    S9xTracef( "Print f: \n");
	}
    }
    for (i = 0; i < 8; i++)
	...
	S9xTracef( "Print f: Read from unknown PPU register: $%04X\n", missing.unknownppu_read);
    if (missing.unknownppu_write)
	S9xTracef( "Print f: Write to unknown PPU register: $%04X\n", missing.unknownppu_write);
    if (missing.unknowncpu_read)
	S9xTracef( "Print f: Read from unknown CPU register: $%04X\n", missing.unknowncpu_read);
    if (missing.unknowncpu_write)
	S9xTracef( "Print f: Write to unknown CPU register: $%04X\n", missing.unknowncpu_write);
    if (missing.unknowndsp_read)
	S9xTracef( "Print f: Read from unknown DSP register: $%04X\n", missing.unknowndsp_read);
    if (missing.unknowndsp_write)
	S9xTracef( "Print f: Write to unknown DSP register: $%04X\n", missing.unknowndsp_write);
}
