...
char *S9xParseArgs (char **argv, int argc)
{
	...
	if (*argv[i] == '-')
	{
		...
		Settings.DisableGraphicWindows = FALSE;
            ...
            else if (strcasecmp (argv [i], "-aidoshm") == 0)
            {
                if (i + 1 < argc)
                {
                    ...
                    S9xTracef( "STDERR: Snes9X running in AIDO mode. shmid: %d\n",
                            Settings.AIDOShmId);
                } else
                    S9xUsage ();
            }
            ...
            else if (strcasecmp (argv [i], "-maxcount") == 0)
            {
                if (i + 1 < argc)
                {
                    ...
                    S9xTracef( "STDERR: Running for a maximum of %d loops.\n",
                            Settings.MaxCount);
                } else
                    S9xUsage ();
            }
		...
		Settings.NetPlay = TRUE;
            ...
            else if (strcasecmp(argv[i],"-minimal")==0){minimal=1;S9xTracef( "Print f: Keyboard with exception of ESC switched off!\n");}
            else if (strcasecmp(argv[i],"-ahiunit")==0)
            {
             if (i+1<argc)
             {
              S9xTracef( "Print f: AHI Unit set to: Unit %i\n",atoi(argv[++i]));
              unit=atoi(argv[++i]);
             }
            }
#endif
	    ...
	    {
              S9xTracef( "Print f: Snes9X " VERSION "\n");
              exit(0);
	    }
...
static void parse_crosshair_spec(enum crosscontrols ctl, const char *spec){
    ...
    if(s[0]=='"'){
        ...
        if(idx>31){
            S9xTracef( "STDERR: Invalid crosshair spec '%s'\n", spec);
            return;
        }
    }
...
void S9xLoadConfigFiles(char **argv, int argc){
    ...
    if(!skip){
        ...
        S9xTracef( "STDERR: Skipping standard config files\n");
    }
...
//    GFX.FrameDisplay=conf.GetBool("Display::DisplayFrameCount", false);
    ...
    if((minimal=conf.GetBool("STORM::Minimal")?1:0)){
        S9xTracef( "Print f: Keyboard with exception of ESC switched off!\n");
    }
