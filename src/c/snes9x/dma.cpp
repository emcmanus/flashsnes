...
static inline bool8 addCyclesInDMA (uint8 dma_channel)
{
	...
	if (CPU.HDMARanInDMA & (1 << dma_channel))
	{
		CPU.HDMARanInDMA = 0;
	#if 1
		S9xTracef( "Print f: HDMA and DMA use the same channel %d!\n", dma_channel);
	#endif
...
bool8 S9xDoDMA (uint8 Channel)
{
    ...
    if (Settings.SPC7110)
	{
		...
		#ifdef SPC7110_DEBUG
			S9xTracef( "Print f: SPC7110: DMA Transfer of %04X bytes from %02X%02X%02X:%02X, offset of %04X, internal bank of %04X, multiplier %02X\n",
	...
	if (Settings.SA1)
	{
		...
		#if 0
			S9xTracef( "Print f: SA-1 DMA: %08x,", base);
			S9xTracef( "Print f: depth = %d, count = %d, bytes_per_char = %d, bytes_per_line = %d, num_chars = %d, char_line_bytes = %d\n",
    ...
    if (d->TransferBytes != 0)
		S9xTracef( "Print f: DMA[%d] TransferBytes not 0! $21%02x Reverse:%d %04x\n", Channel, d->BAddress, d->ReverseTransfer, d->TransferBytes);
#endif
