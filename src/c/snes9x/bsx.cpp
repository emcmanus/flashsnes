...
{
#ifdef BSX_DEBUG
	S9xTracef( "Print f: BS: Remapping\n");
	for (int i = 0; i < 32; i++)
		S9xTracef( "Print f: BS: MMC %02X: %d\n", i, BSX.MMC[i]);
#endif
...
#ifdef BSX_DEBUG
	if (r)
		S9xTracef( "Print f: BS: BIOS found.\n");
	else
		S9xTracef( "Print f: BS: BIOS not found!\n");
#endif
	...
	else
	{
		...
		if (Settings.BS)
		{
			...
			for (int i = 0; i <= 0x1F; i++)
				S9xTracef( "Print f: BS: ROM Header %02X: %02X\n", i, header[i]);
			S9xTracef( "Print f: BS: FlashMode: %d, FlashSize: %x\n", FlashMode, FlashSize);
#endif
...
#ifdef BSX_DEBUG
		S9xTracef( "Print f: BS: Current Time: %02d:%02d:%02d\n",  BSX_RTC.hours, BSX_RTC.minutes, BSX_RTC.seconds);
#endif
