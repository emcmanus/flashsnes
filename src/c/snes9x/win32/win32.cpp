...
//S9xTracef( "Print f: play_pos = %d, write_pos = %d, curr_block = %d, lastblock = %d\n",
	...
	// Wait for heart beat from server
        ...
        else
        {
            if (success)
            {
                ...
                if (prev > 1)
                {
                    S9xTracef( "Print f: CLIENT: SyncSpeed prev: %d @%d\n", prev, timeGetTime () - START);
                }
#endif
            }
            else
            {
#ifdef NP_DEBUG
                S9xTracef( "Print f: *** CLIENT: SyncSpeed: Release failed @ %d\n", timeGetTime () - START);
#endif
            }
