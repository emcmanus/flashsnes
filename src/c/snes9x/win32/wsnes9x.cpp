...
void ToggleFullScreen ()
{
    ...
    if (VOODOO_MODE)
    {
		...
		else
		{
			...
			if (!DirectX.SetDisplayMode (GUI.Width, GUI.Height, max(GetFilterScale(GUI.Scale), GetFilterScale(GUI.ScaleHiRes)), GUI.Depth, GUI.RefreshRate, GUI.FullScreen, GUI.DoubleBuffered))
			{
...
/*					if(!Settings.UpAndDown)
	...
	if(!(wParam == 0 || wParam == VK_ESCAPE)) // if it's the 'disabled' key, it's never pressed as a hotkey
	{
		...
		&& modifiers == CustomKeys.FrameAdvance.modifiers)
		{
			...
			if((int)(timeGetTime() - lastTime) > 20 && (!ICPU.SavedAtOp || prevPadReadFrame == IPPU.TotalEmulatedFrames))
			{
...
//						Settings.FrameTime / 1);
		...
		if(wParam == CustomKeys.GLCube.key
		&& modifiers == CustomKeys.GLCube.modifiers)
		{
			#ifdef USE_OPENGL
...
//				bool old_stretch = GUI.Stretch;
				...
				{
					UpdateScale(GUI.Scale, GUI.NextScale);
				}
...
static void CheckMenuStates ()
{
    ...
    if (!DirectX.DSAvailable || Settings.SoundPlaybackRate == 0 || Settings.Mute || !Settings.APUEnabled)
        SetMenuItemInfo (GUI.hMenu, ID_SOUND_NOSOUND, FALSE, &mii);
    else
    {
        for (i = 0; i < COUNT(SoundRates); i++)
...
//		prevVideoMemory = GUI.VideoMemory;
		...
		sprintf(s,"Current Mode: %dx%d %dbit %dHz",GUI.Width,GUI.Height,GUI.Depth,GUI.RefreshRate);
...
//		EnableWindow(GetDlgItem(hDlg, IDC_RENDER16BIT), FALSE);
...
//		EnableWindow(GetDlgItem(hDlg, IDC_BILINEARMD7), FALSE);
//		{
////			SendDlgItemMessage(hDlg, IDC_BILINEARMD7, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
//			SendDlgItemMessage(hDlg, IDC_BILINEARMD7, BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);
//		}
		...
		EnableWindow(GetDlgItem(hDlg, IDC_LOCALVIDMEM), GUI.VideoMemory);
		if(GUI.LocalVidMem)
...
//		SendDlgItemMessage(hDlg,IDC_FILTERBOX2,CB_SETCURSEL,(WPARAM)GUI.NextScaleHiRes,0);
		...
		DirectX.lpDD->EnumDisplayModes(DDEDM_REFRESHRATES,NULL,GetDlgItem(hDlg,IDC_VIDMODELIST),(LPDDENUMMODESCALLBACK)EnumModesCallback);

		// have to start focus on something like this or Escape won't exit the dialog
		SetFocus(GetDlgItem(hDlg,IDC_FILTERBOX));
...
//			Settings.SixteenBit = IsDlgButtonChecked(hDlg, IDC_RENDER16BIT);
			...
			GUI.VideoMemory = (bool)(IsDlgButtonChecked(hDlg,IDC_VIDEOCARD)==BST_CHECKED);
			GUI.LocalVidMem = (bool)(IsDlgButtonChecked(hDlg,IDC_LOCALVIDMEM)==BST_CHECKED);
			if(!GUI.FullScreen || (GUI.Width >= 512 && GUI.Height >= 478) || GUI.Stretch)
...
//				GUI.VideoMemory = prevVideoMemory;
...
//				if(DirectX.Clipped) S9xReRefresh();
		...
		case IDC_SETDMODE:
			...
			if(go)
			{
				...
				GUI.RefreshRate=dm.at(index).depth;
			}
			if(!GUI.FullScreen || (GUI.Width >= 512 && GUI.Height >= 478) || GUI.Stretch)
			{
				EnableWindow(GetDlgItem(hDlg,IDC_FILTERBOX),true);
...
int CALLBACK DlgOpenMovie(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		if(DirectX.Clipped) S9xReRefresh();
		{
			...
			EnableWindow(GetDlgItem(hDlg, IDC_SYNC_TO_SOUND_CPU), 0==Settings.SoundDriver); // can't sync sound to CPU unless using "Snes9x DirectSound" driver
...
static void WinDisplayString (const char *string, int linesFromBottom, int pixelsFromLeft, bool allowWrap)
{
	if(GUI.ScreenDepth == 32 && GUI.DepthConverted)
	{
