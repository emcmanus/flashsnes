...
{
 S9xTracef( "Print f: Failure opening %s for %s\n",filename,mode);
 return 1;
}
...
int main(int argc, char **argv)
{
 ...
 if (argc < 2 || argc > 3)
 {
  S9xTracef( "Print f: Fixes MS Win32 object files to be compatible with the incorrect\n");
  S9xTracef( "Print f:  implementation in MinGW32.\n");
  S9xTracef( "Print f: Usage: objfix infile [outfile]\n");
  return 1;
 }
