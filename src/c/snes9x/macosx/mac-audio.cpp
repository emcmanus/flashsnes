	...
	if (aueffect)
	{
		...
		*/
		...
		if ((aueffect & kAUReverb) && (aueffect & kAUGraphEQ))
		{
			...
			S9xTracef( "Print f: AudioUnit Connection: Output <- Reverb <- GraphEQ <- Converter <- Snes9x\n");
		}
		else
		if (aueffect & kAUReverb)
		{
			...
			S9xTracef( "Print f: AudioUnit Connection: Output <- Reverb <- Converter <- Snes9x\n");
		}
		else
		if (aueffect & kAUGraphEQ)
		{
			...
			S9xTracef( "Print f: AudioUnit Connection: Output <- GraphEQ <- Converter <- Snes9x\n");
		}
		...
		*/
		...
		S9xTracef( "Print f: AudioUnit Connection: Output <- Snes9x\n");

	// Tell Snes9x's format to target AudioUnit

	SetAudioUnitSoundFormatAndVolume();

	//S9xTracef( "Print f: Connected AudioUnits\n");
}
...
static void DisconnectAudioUnits(void)
{
	...
	//S9xTracef( "Print f: Disconnected AudioUnits\n");
}
...
void MacStartSound(void)
{
	...
	if (!soundstarted)
	{
		...
		S9xTracef( "Print f: AudioUnit output started.\n");
	}
}
...
void MacStopSound(void)
{
	...
	if (soundstarted)
	{
		...
		S9xTracef( "Print f: AudioUnit output stopped.\n");
	}
}
...
static void SetAudioUnitSoundFormatAndVolume(void)
{
	...
	/*
	if (target == output)
		S9xTracef( "Print f: target == output\n");
	else
		S9xTracef( "Print f: target == converter\n");
	*/
	...
	*/
	...
	//S9xTracef( "Print f: Set format and volume \n");
}
...
static void ChangeAudioDeviceBufferFrameSize(UInt32 newvalue, UInt32 *old)
{
	...
	{
		S9xTracef( "Print f: Using default output buffer size.\n");
		return;
	}
...
bool8 S9xOpenSoundDevice(int mode, bool8 stereo, int buffer_size)
{
	...
	//S9xTracef( "Print f: Aligned samplesPerSlice:%d\n", samplesPerSlice);
