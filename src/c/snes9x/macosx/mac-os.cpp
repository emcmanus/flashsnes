...
int main(int argc, char **argv)
{
	...
	while (!finished)
	{
		if (cartOpen && running)
		{
			...
			if (macFrameSkip < 0)
			{
				S9xTracef( "Print f: Frame skip: auto\n");
				skipFrames = 3;
			}
			else
			{
				S9xTracef( "Print f: Frame skip: %d\n", macFrameSkip - 1);
...
static inline void EmulationLoop(void)
{
	...
	S9xTracef( "Print f: Minimize CPU usage: %s\n", minimizecpu ? "on" : "off");
						...
						switch (GetEventKind(inEvent))
						{
							case kEventMouseUp:
								...
								if (err == noErr)
								{
									...
									{
										S9xTracef( "Print f: Exiting from fullscreen...\n");
...
static void Initialize(void)
{
	...
	S9xTracef( "Print f: PowerPC\n\n");
#else
	S9xTracef( "Print f: Intel\n\n");
#endif
...
{
	S9xTracef( "Print f: Quit. %s  err: %ld\n", msg, err);
	ExitToShell();
}
