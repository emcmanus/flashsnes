...
void S9xControlsSoftReset(void){
	...
	// Commenting this loop - it's causing the VM to hang. Why is 9x deleting the iterator and then trying to increment it? - Ed
...
void S9xSetController(int port, enum controllers controller, int8 id1, int8 id2, int8 id3, int8 id4){
    ...
    switch(controller){
      ...
      case CTL_MP5:
        ...
        return;
      default:
        S9xTracef( "STDERR: Unknown controller type %d\n", controller);
        break;
    }
...
s9xcommand_t S9xGetCommandT(const char *name){
    ...
    if(!strcmp(name,"None")){
        ...
        if(multis.size()>2147483640){
            S9xTracef( "STDERR: Too many multis!");
            return cmd;
        }
        string x;
        int n;
        ...
        for(i=(name[0]=='+')?2:1; name[i]!='\0'; i++){
            if(name[i]==',' || name[i]==';'){
                if(name[i]==';') j++;
                if(++j>2147483640){
                    S9xTracef( "STDERR: Multi too long!");
                    return cmd;
                }
            }
...
bool S9xMapButton(uint32 id, s9xcommand_t mapping, bool poll){
    ...
    if(id==InvalidControlID){
        S9xTracef( "STDERR: Cannot map InvalidControlID\n");
        return false;
    }
    ...
    if(t!=MAP_NONE && t!=MAP_BUTTON) S9xTracef( "STDERR: WARNING: Remapping ID 0x%08x from %s to button\n", id, maptypename(t));
    if(id>=PseudoPointerBase){
        S9xTracef( "STDERR: ERROR: Refusing to map pseudo-pointer #%d as a button\n", id-PseudoPointerBase);
        return false;
    }
    ...
    if(poll){
        if(id>=PseudoButtonBase){
            S9xTracef( "STDERR: INFO: Ignoring attempt to set pseudo-button #%d to polling\n", id-PseudoButtonBase);
...
bool S9xMapPointer(uint32 id, s9xcommand_t mapping, bool poll){
    ...
    t=maptype(S9xGetMapping(id).type);
    if(t!=MAP_NONE && t!=MAP_POINTER) S9xTracef( "STDERR: WARNING: Remapping ID 0x%08x from %s to pointer\n", id, maptypename(t));
    if(id<PseudoPointerBase && id>=PseudoButtonBase){
        S9xTracef( "STDERR: ERROR: Refusing to map pseudo-button #%d as a pointer\n", id-PseudoButtonBase);
        return false;
    }

    if(mapping.type==S9xPointer){
        if(mapping.pointer.aim_mouse0 && mouse[0].ID!=InvalidControlID && mouse[0].ID!=id){ S9xTracef( "STDERR: ERROR: Rejecting attempt to control Mouse1 with two pointers\n"); return false; }
        if(mapping.pointer.aim_mouse1 && mouse[1].ID!=InvalidControlID && mouse[1].ID!=id){ S9xTracef( "STDERR: ERROR: Rejecting attempt to control Mouse2 with two pointers\n"); return false; }
        if(mapping.pointer.aim_scope && superscope.ID!=InvalidControlID && superscope.ID!=id){ S9xTracef( "STDERR: ERROR: Rejecting attempt to control SuperScope with two pointers\n"); return false; }
        if(mapping.pointer.aim_justifier0 && justifier.ID[0]!=InvalidControlID && justifier.ID[0]!=id){ S9xTracef( "STDERR: ERROR: Rejecting attempt to control Justifier1 with two pointers\n"); return false; }
        if(mapping.pointer.aim_justifier1 && justifier.ID[1]!=InvalidControlID && justifier.ID[1]!=id){ S9xTracef( "STDERR: ERROR: Rejecting attempt to control Justifier2 with two pointers\n"); return false; }
    }

    S9xUnmapID(id);
    if(poll){
        if(id>=PseudoPointerBase){
            S9xTracef( "STDERR: INFO: Ignoring attempt to set pseudo-pointer #%d to polling\n", id-PseudoPointerBase);
        } else switch(mapping.type){
          case S9xPointer:
...
void S9xReportPointer(uint32 id, int16 x, int16 y){
    ...
    if(maptype(keymap[id].type)!=MAP_POINTER){
        S9xTracef( "STDERR: ERROR: S9xReportPointer called on %s ID 0x%08x\n", maptypename(maptype(keymap[id].type)), id);
        return;
    }
...
bool S9xMapAxis(uint32 id, s9xcommand_t mapping, bool poll){
    ...
    t=maptype(S9xGetMapping(id).type);
    if(t!=MAP_NONE && t!=MAP_AXIS) S9xTracef( "STDERR: WARNING: Remapping ID 0x%08x from %s to axis\n", id, maptypename(t));
    if(id>=PseudoPointerBase){
        S9xTracef( "STDERR: ERROR: Refusing to map pseudo-pointer #%d as an axis\n", id-PseudoPointerBase);
        return false;
    }
    if(id>=PseudoButtonBase){
        S9xTracef( "STDERR: ERROR: Refusing to map pseudo-button #%d as an axis\n", id-PseudoButtonBase);
        return false;
    }
...
void S9xReportAxis(uint32 id, int16 value){
    ...
    if(maptype(keymap[id].type)!=MAP_AXIS){
        S9xTracef( "STDERR: ERROR: S9xReportAxis called on %s ID 0x%08x\n", maptypename(maptype(keymap[id].type)), id);
        return;
    }
    ...
    switch(cmd.type){
      ...
      case S9xButtonCommand:
        if(((enum command_numbers)cmd.button.command)>=LAST_COMMAND){
            S9xTracef( "STDERR: Unknown command %04x\n", cmd.button.command);
            return;
        }
        if(!data1){
...
/*              case Mode7Interpolate:
      ...
      default:
        S9xTracef( "STDERR: WARNING: Unknown command type %d\n", cmd.type);
        return;
    }
}
...
void S9xSetControllerCrosshair(enum crosscontrols ctl, int8 idx, const char *fg, const char *bg){
    ...
    if(idx<-1 || idx>31){
        S9xTracef( "STDERR: S9xSetControllerCrosshair() called with invalid index\n");
        return;
    }
    ...
    switch(ctl){
      ...
      default:
        S9xTracef( "STDERR: S9xSetControllerCrosshair() called with an invalid controller ID %d\n", ctl);
        return;
    }
    ...
    if(fg!=NULL){
        ...
        fgcolor|=i;
        if(i>15 || fgcolor==16){
            S9xTracef( "STDERR: S9xSetControllerCrosshair() called with invalid fgcolor\n");
            return;
        }
    }
    ...
    if(bg!=NULL){
        ...
        bgcolor|=i;
        if(i>15 || bgcolor==16){
            S9xTracef( "STDERR: S9xSetControllerCrosshair() called with invalid bgcolor\n");
            return;
        }
    }
...
void S9xGetControllerCrosshair(enum crosscontrols ctl, int8 *idx, const char **fg, const char **bg){
    ...
    switch(ctl){
      ...
      case X_JUSTIFIER2: c=&justifier.crosshair[1]; break;
      default:
        S9xTracef( "STDERR: S9xGetControllerCrosshair() called with an invalid controller ID %d\n", ctl);
        return;
    }
