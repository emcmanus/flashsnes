...
#ifdef DEBUGGER
    if(Memory.C4RAM[0x625]!=0) S9xTracef( "Print f: $6625=%02x, expected 00\n", Memory.C4RAM[0x625]);
    if((Memory.C4RAM[0x626]>>2)!=Memory.C4RAM[0x629]) S9xTracef( "Print f: $6629=%02x, expected %02x\n", Memory.C4RAM[0x629], (Memory.C4RAM[0x626]>>2));
    if(((uint16)Memory.C4RAM[0x626]<<2)!=READ_WORD(Memory.C4RAM+0x627)) S9xTracef( "Print f: $6627=%04x, expected %04x\n", READ_WORD(Memory.C4RAM+0x627), ((uint16)Memory.C4RAM[0x626]<<2));
#endif
...
//    S9xTracef( "Print f: %dx%d XScale=%04x YScale=%04x angle=%03x\n", w, h, XScale, YScale, READ_WORD(Memory.C4RAM+0x1f80)&0x1ff);
//    S9xTracef( "Print f: Matrix: [%10g %10g]  [%04x %04x]\n", A/4096.0, B/4096.0, A&0xffff, B&0xffff);
//    S9xTracef( "Print f:         [%10g %10g]  [%04x %04x]\n", C/4096.0, D/4096.0, C&0xffff, D&0xffff);
...
#ifdef DEBUGGER
    if(Memory.C4RAM[0x1f97]!=0) S9xTracef( "Print f: $7f97=%02x, expected 00\n", Memory.C4RAM[0x1f97]);
    if((Cx&~1)!=w/2 || (Cy&~1)!=h/2) S9xTracef( "Print f: Center is not middle of image! (%d, %d) != (%d, %d)\n", Cx, Cy, w/2, h/2);
#endif
...
#ifdef DEBUGGER
    if(READ_3WORD(Memory.C4RAM+0x1f8f)&0xff00ff) S9xTracef( "Print f: wireframe: Unexpected value in $7f8f: %06x\n", READ_3WORD(Memory.C4RAM+0x1f8f));
    if(READ_3WORD(Memory.C4RAM+0x1fa4)!=0x001000) S9xTracef( "Print f: wireframe: Unexpected value in $7fa4: %06x\n", READ_3WORD(Memory.C4RAM+0x1fa4));
#endif
...
#ifdef DEBUGGER
    if(Memory.C4RAM[0x1f8a]!=0x90) S9xTracef( "Print f: lines: $7f8a = %02x, expected 90\n", READ_WORD(Memory.C4RAM+0x1f8a));
#endif
...
#ifdef DEBUGGER
    if(READ_3WORD(Memory.C4RAM+0x1f80) != Memory.C4RAM[waveptr+0xb00]) S9xTracef( "Print f: $7f80=%06x, expected %02x\n", READ_3WORD(Memory.C4RAM+0x1f80), Memory.C4RAM[waveptr+0xb00]);
#endif
...
#ifdef DEBUGGER
    if((Cx&~1)!=width/2 || (Cy&~1)!=height/2) S9xTracef( "Print f: Center is not middle of image for disintegrate! (%d, %d) != (%d, %d)\n", Cx, Cy, width/2, height/2);
#endif
...
static void S9xC4ProcessSprites()
{
    switch(Memory.C4RAM[0x1f4d])
	{
      case 0x00: // Build OAM
#ifdef DEBUGGER
//        S9xTracef( "Print f: 00 00 Build OAM!\n");
#endif
        C4ConvOAM();
        break;
...
#ifdef DEBUGGER
//        S9xTracef( "Print f: 00 03 Scale/Rotate!\n");
#endif
        C4DoScaleRotate(0);
        break;
...
#ifdef DEBUGGER
//        S9xTracef( "Print f: 00 05 Transform Lines!\n");
#endif
        C4TransformLines();
        break;
...
#ifdef DEBUGGER
//        S9xTracef( "Print f: 00 07 Scale/Rotate!\n");
#endif
        C4DoScaleRotate(64);
        break;
...
#ifdef DEBUGGER
//        S9xTracef( "Print f: 00 08 Draw wireframe!\n");
#endif
        C4DrawWireFrame();
        break;
...
#ifdef DEBUGGER
        S9xTracef( "Print f: 00 0b Disintegrate!\n");
#endif
        C4SprDisintegrate();
        break;
...
//        S9xTracef( "Print f: 00 0b Wave!\n");
      ...
      default:
#ifdef DEBUGGER
        S9xTracef( "Print f: Unknown C4 sprite command (%02x)\n", Memory.C4RAM [0x1f4d]);
#endif
        break;
    }
}
...
void S9xSetC4 (uint8 byte, uint16 Address)
{
    ...
    if (Address == 0x7f4f)
	{
		...
		{
#ifdef DEBUGGER
            S9xTracef( "Print f: Test command %02x 0e used!\n", byte);
#endif
...
#ifdef DEBUGGER
                //S9xTracef( "Print f: 01 Draw wireframe used!\n");
                if(Memory.C4RAM[0x1f4d]!=8) S9xTracef( "Print f: $7f4d=%02x, expected 08 for command 01 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
...
#ifdef DEBUGGER
                S9xTracef( "Print f: 05 Propulsion (?)!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 05 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
                    int32 tmp=0x10000;
...
#ifdef DEBUGGER
                S9xTracef( "Print f: 0d Set vector length!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 0d %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
...
#ifdef DEBUGGER
//                S9xTracef( "Print f: 10 Polar->Rect!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 10 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
...
#ifdef DEBUGGER
//                S9xTracef( "Print f: 13 Polar->Rect!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 13 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
...
#ifdef DEBUGGER
                S9xTracef( "Print f: 15 Pythagorean!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 15 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                C41FXVal=READ_WORD(Memory.C4RAM+0x1f80);
                C41FYVal=READ_WORD(Memory.C4RAM+0x1f83);
				//C4Op15(); // optimized to:
              ...
              case 0x1f: // atan
#ifdef DEBUGGER
//                S9xTracef( "Print f: 1f atan!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 1f %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                C41FXVal=READ_WORD(Memory.C4RAM+0x1f80);
                C41FYVal=READ_WORD(Memory.C4RAM+0x1f83);
                C4Op1F();
              ...
              case 0x22: // Trapezoid
                {
#ifdef DEBUGGER
//                   S9xTracef( "Print f: 22 Trapezoid!\n");
                    if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 22 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                    int16 angle1=READ_WORD(Memory.C4RAM+0x1f8c)&0x1ff;
                    int16 angle2=READ_WORD(Memory.C4RAM+0x1f8f)&0x1ff;
#ifdef DEBUGGER
                    if(C4CosTable[angle1]==0) S9xTracef( "STDERR: 22 Trapezoid: Invalid tangent! angle1=%d\n", angle1);
                    if(C4CosTable[angle2]==0) S9xTracef( "STDERR: 22 Trapezoid: Invalid tangent! angle2=%d\n", angle2);
#endif
              ...
              case 0x25: // Multiply
#ifdef DEBUGGER
                S9xTracef( "Print f: 25 Multiply!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 25 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
...
#ifdef DEBUGGER
//                S9xTracef( "Print f: 2d Transform Coords!\n");
                if(Memory.C4RAM[0x1f4d]!=2) S9xTracef( "Print f: $7f4d=%02x, expected 02 for command 2d %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
                if(READ_3WORD(Memory.C4RAM+0x1f8f)&0xff00ff) S9xTracef( "Print f: 2d transform coords: Unexpected value in $7f8f: %06x\n", READ_3WORD(Memory.C4RAM+0x1f8f));
                if(READ_3WORD(Memory.C4RAM+0x1f8c)!=0x001000) S9xTracef( "Print f: 0d transform coords: Unexpected value in $7f8c: %06x\n", READ_3WORD(Memory.C4RAM+0x1f8c));
#endif
              ...
              case 0x40: // Sum
#ifdef DEBUGGER
                S9xTracef( "Print f: 40 Sum!\n");
                if(Memory.C4RAM[0x1f4d]!=0x0e) S9xTracef( "Print f: $7f4d=%02x, expected 0e for command 40 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
                    uint16 sum=0;
              ...
              case 0x54: // Square
#ifdef DEBUGGER
                S9xTracef( "Print f: 54 Square!\n");
                if(Memory.C4RAM[0x1f4d]!=0x0e) S9xTracef( "Print f: $7f4d=%02x, expected 0e for command 54 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                {
                    int64 a=SAR((int64)READ_3WORD(Memory.C4RAM+0x1f80)<<40, 40);
				//	S9xTracef( "Print f: %08X%08X\n", (uint32)(a>>32), (uint32)(a&0xFFFFFFFF));
                    a*=a;
				//	S9xTracef( "Print f: %08X%08X\n", (uint32)(a>>32), (uint32)(a&0xFFFFFFFF));
...
#ifdef DEBUGGER
                S9xTracef( "Print f: 5c Immediate Reg!\n");
                if(Memory.C4RAM[0x1f4d]!=0x0e) S9xTracef( "Print f: $7f4d=%02x, expected 0e for command 5c %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
                for (i = 0; i < 12 * 4; i++)
...
#ifdef DEBUGGER
                S9xTracef( "Print f: 89 Immediate ROM!\n");
                if(Memory.C4RAM[0x1f4d]!=0x0e) S9xTracef( "Print f: $7f4d=%02x, expected 0e for command 89 %02x\n", Memory.C4RAM[0x1f4d], Memory.C4RAM[0x1f4d]);
#endif
              ...
              default:
#ifdef DEBUGGER
                S9xTracef( "Print f: Unknown C4 command (%02x)\n", byte);
#endif
                break;
            }
        }
...
#ifdef DEBUGGER
//        S9xTracef( "Print f: C4 load memory %06x => %04x, %04x bytes\n", READ_3WORD(Memory.C4RAM+0x1f40), READ_WORD(Memory.C4RAM+0x1f45), READ_WORD(Memory.C4RAM+0x1f43));
        if(byte != 0) S9xTracef( "Print f: C4 load: non-0 written to $7f47! Wrote %02x\n", byte);
        if(READ_WORD(Memory.C4RAM+0x1f45) < 0x6000 || (READ_WORD(Memory.C4RAM+0x1f45) + READ_WORD(Memory.C4RAM+0x1f43)) > 0x6c00) S9xTracef( "Print f: C4 load: Dest unusual! It's %04x\n", READ_WORD(Memory.C4RAM+0x1f45));
#endif
