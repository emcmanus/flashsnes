...
static void Convert16to15(void)
{
  for (register int y = 0; y < SNES_HEIGHT_EXTENDED; y++) {
    ...
    register uint16 *s = (uint16 *) (GFX.Screen + y);
...
static void Convert8to8(void)
{
  for (register int y = 0; y < SNES_HEIGHT_EXTENDED; y++)
  {
    ggiPutHLine(vis, 0,y, SNES_WIDTH, (GFX.Screen + y));
  }
}
static void Convert16to8(void)
{
  S9xTracef( "STDERR: 16 -> 8 bit conversion not implemented yet.\n");
}
#undef DEFINE_DEST
...
void S9xGraphicsMode ()
{
    if (!vis)
    {
        if(!(vis = ggiOpen(NULL)))
        {
            S9xTracef( "STDERR: ggiOpen() failed.\n");
            S9xExit();
        }
    }
    ...
    if(text_mode)
    {
        S9xTracef( "STDERR: %d: ", __LINE__); ggiFPrintMode(stderr, &vis_mode); S9xTracef( "STDERR: \n");
...
//      S9xExit();
        ...
        {
            S9xTracef( "STDERR: ggiSetMode() failed.\n");
            S9xExit();
        }
...
void S9xInitDisplay (int argc, char **argv)
{
    ...
    if(ggiInit() != 0)
    {
        S9xTracef( "STDERR: ggiInit() failed.\n");
        S9xExit();
    }
...
void S9xProcessEvents (bool8 block)
{
    ...
    for(;;)
    {
        ...
        if(pause_mode)
        {
            ...
            S9xTracef( "STDERR: Resume by key %d (%c)\n", c, c);
            ...
            if(text_mode)
            {
                S9xTracef( "STDERR: back: ggiInit: %d\n", ggiInit());
                vis = 0;
        ...
        if(down)
        {
            ...
            if(event.key.label == '1')Settings.BG_Forced ^= 1;
            if(event.key.label == '2')Settings.BG_Forced ^= 2;
            if(event.key.label == '3')Settings.BG_Forced ^= 4;
            if(event.key.label == '4')Settings.BG_Forced ^= 8;
            if(event.key.label == '5')Settings.BG_Forced ^= 16;
            ...
            if(event.key.label=='P')
            {
                ...
                {
                    S9xTracef( "STDERR: Alt: %d, Shift: %d, Ctrl: %d\n",
                            alt,shift,control);
