...
void S9xGraphicsMode ()
{
	...
	if (ret < 0)
	{
	    S9xTracef( "STDERR: Unable to switch to requested screen mode/resolution:\n");
	    S9xExit ();
	}
...
void S9xPutImage (int width, int height)
{
    ...
    if (planar)
	vga_copytoplanar256 ((uint8 *)GFX.Screen + y_buff * GFX.Pitch,
			     IMAGE_WIDTH,
	...
	if (screen_pitch == width && screen_height >= height)
	{
	    ...
	    uint32 *end = (uint32 *) ((uint8 *)GFX.Screen + width * height);
	    do
	    {
		if (*n != *o)
		    *o = *s = *n;
	...
	else
	{
		...
		for (int y = 0; y < height; y++)
		{
		    ...
		    register uint8 *s = (uint8 *)GFX.Screen + yy * GFX.Pitch;
		    y_error += y_fraction;
