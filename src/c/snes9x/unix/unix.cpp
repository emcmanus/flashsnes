...
int main (int argc, char **argv)
{
	...
	if (!loaded && rom_filename)
	{
	    ...
	    {
		S9xTracef( "STDERR: Error opening: %s\n", rom_filename);
		exit (1);
	    }
		loaded = TRUE;
	}
	...
	if (!S9xNPConnectToServer(Settings.ServerName, Settings.Port,
					Memory.ROMName))
	{
	    S9xTracef( "STDERR: Failed to connect to Snes9x netplay"
                     " server \"%s\" on port %d.\n",
                     Settings.ServerName, Settings.Port);
	    S9xExit ();
	}
	S9xTracef( "STDERR: Connected to \"%s\" on port %d as"
                 " player #%d playing \"%s\"\n",
	...
	if (NP_Activated)
	{
		...
		{
			S9xTracef( "STDERR: Lost netplay connection to server\n");
			S9xExit();
		}
	}
#endif
...
void InitJoysticks ()
{
    ...
    if ((js_fd [0] = open (js_device [0], O_RDONLY | O_NONBLOCK)) < 0)
    {
        ...
        perror (js_device [0]);
#endif
        S9xTracef( "STDERR: joystick: No joystick found.\n");
	return;
    }

    if (ioctl (js_fd [0], JSIOCGVERSION, &version))
    {
        S9xTracef( "STDERR: joystick: You need at least driver version 1.0"
    ...
    {
        S9xTracef( "Print f: Using %s (%s) as joystick1\n", name, js_device [0]);
        for(j=1; j<8; j++){
            if (js_fd [j] > 0)
            {
                ioctl (js_fd [j], JSIOCGNAME(128), name);
                S9xTracef( "Print f:   and %s (%s) as joystick%d\n", name, js_device [j], j+1);
            }
        }
    }
    else
	...
	S9xTracef( "Print f: Using %d-axis %d-button joystick (%s) as joystick1\n", axes, buttons, js_device [0]);
        for(j=1; j<8; j++){
            if (js_fd [j] > 0)
            {
                ioctl (js_fd [j], JSIOCGAXES, &axes);
                ioctl (js_fd [j], JSIOCGBUTTONS, &buttons);
                S9xTracef( "Print f:   and %d-axis %d-button joystick (%s) as joystick%d\n", axes, buttons, js_device [j], j+1);
            }
        }
    }
#endif
...
//	S9xTracef( "Print f: \rFrame rate: %.2lfms", (double) (here - then) / 60);
//	fflush (stdout);
	then = here;
    }
}
...
void S9xSyncSpeed ()
{
	...
	S9xTracef( "Print f: CLIENT: SyncSpeed @%d\n", S9xGetMilliTime());
#endif
...
#if defined (NP_DEBUG) && NP_DEBUG == 2
	    if (NetPlay.PendingWait4Sync)
	        S9xTracef( "Print f: CLIENT: PendingWait4Sync1 @%d\n", S9xGetMilliTime());
#endif
...
#if defined (NP_DEBUG) && NP_DEBUG == 2
	    if (NetPlay.PendingWait4Sync)
	        S9xTracef( "Print f: CLIENT: PendingWait4Sync2 @%d\n", S9xGetMilliTime());
#endif
...
#  if 0
	S9xTracef( "Print f: channels now = %ld\n", channels);
#  endif
#endif
	...
	S9xTracef( "Print f: channels = %ld\n", channels);
	S9xTracef( "Print f: sampsize = %ld\n", sampsize);
	S9xTracef( "Print f: samprate = %ld\n", samprate);
	S9xTracef( "Print f: outsize =  %ld\n", outsize);
	S9xTracef( "Print f: compress = %ld\n", compress);
	S9xTracef( "Print f: device =   %ld\n", device);
	S9xTracef( "Print f: interleave=%ld\n", interleave);
#endif
	...
	samprate = al_params[1];
# if 0
	S9xTracef( "Print f: samprate now = %ld\n", samprate);
#endif
#endif
	...
	sampsize = ALgetwidth(al_config);
	S9xTracef( "Print f: sampsize now = %ld\n", sampsize);
#endif
	...
	S9xTracef( "Print f: SGI sound successfully opened\n");
	S9xTracef( "Print f:   Rate: %d, Buffer size: %d, 16-bit: %s, Stereo: %s, Encoded: %s\n",
...
void S9xUnixProcessSound (void)
{
	...
	S9xTracef( "Print f: why am I hear? alport = %p\n", so.al_port);
	return;
    }
...
#if 0
	S9xTracef( "Print f: error: fillable space only = %ld\n", ALgetfillable(so.al_port));
#endif
	return;
    }
...
#if 0
	    S9xTracef( "Print f: about to write buffer %p size %d\n", audio_buf, buf_size);
#endif
...
bool8 S9xOpenSoundDevice (int mode, bool8 stereo, int buffer_size)
{
    ...
    S9xTracef( "Print f: Sound Device: %s, Rate: %d, Buffer size: %d, 16-bit: %s, Stereo: %s, Encoded: %s\n",
	    sound_device,
	    so.playback_rate, so.buffer_size, so.sixteen_bit ? "yes" : "no",
	    so.stereo ? "yes" : "no", so.encoded ? "yes" : "no");
...
bool8 S9xOpenSoundDevice (int mode, bool8 stereo, int buffer_size)
{
    ...
    {
	if (J & DSP_CAP_MMAP)
	    S9xTracef( "Print f: DSP_CAP_MMAP supported\n");
    }
    void *ptr;

    if ((ptr = mmap (0, so.buffer_size * 3, PROT_WRITE, 0, so.sound_fd, 0)) == 0)
	S9xTracef( "STDERR: mmap failed\n");
...
void *S9xProcessSound (void *)
{
    ...
    {
	S9xTracef( "Print f: F"); fflush (stdout);
    }
    else
    {
	S9xTracef( "Print f: <%d,%d>", count.blocks, count.bytes); fflush (stdout);
    	return (NULL);
    }
#endif
	...
	unsigned byte_offset = so.play_position +
		      (so.sixteen_bit ? (so.samples_mixed_so_far << 1)
				      : so.samples_mixed_so_far);
//S9xTracef( "Print f: %d:", sample_count - so.samples_mixed_so_far); fflush (stdout);
	if (Settings.SoundSync == 2)
	{
...
void S9xLoadSDD1Data ()
{
	...
	{
	    S9xTracef( "STDERR: Decompressed data pack not found in '%s'.\n",
                     filename);
	}
    }
}
