...
void S9xAIDODeinit ()
{
  S9xTracef( "STDERR: Snes9X exited. This shouldn't happen.\n");
}
...
void S9xAIDOPutImage (int snes_width, int snes_height)
{
  ...
  //  S9xTracef( "STDERR: data: %p   offset:%d   size:%d\n", data, offset, size);
...

  {
    ...
    if( snes_width != ow )
    {
      S9xTracef( "Print f: mem_w:%d\n", snes_width);
      ow = snes_width;
    }
    if( snes_height != oh )
    {
      S9xTracef( "Print f: mem_h:%d\n", snes_height);
      oh = snes_height;
    }
  }

  S9xTracef( "Print f: mem_offset:%d\n", offset);
  fflush(stdout);
  offset += size;
}
...
static void handle_controller (char *input)
{
  ...
  //  S9xTracef( "STDERR: Handling controller %d.\n", controller);
  ...
  switch (button)
  {
   ...
   case 'h':
     byte1 = 16;	break; // RT
   default:
     S9xTracef( "STDERR: Snes9X: Unsupported command %d.\n", button);
  }
...
void S9xAIDOProcessEvents (bool8 block)
{
  ...
  do {
    ...
    {
      S9xTracef( "STDERR: Snes9X: Input poll failed. Exiting.\n");
      exit(1);
    }
    ...
    {
      if(errno == EBADF) {
        S9xTracef( "STDERR: Snes9X: Bad fd. Exiting.\n");
        exit(1);
      }
      return;
    }
