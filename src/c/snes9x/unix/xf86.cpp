...
void S9xInitDisplay (int argc, char **argv)
{
	...
	if ((ourdisp = XOpenDisplay(NULL)) == NULL) {
		S9xTracef( "Print f: Can't connect to X server!\n");
		S9xExit ();
	}
	...
	switch (ourvideo.bitdepth) {
		...
		case 24:
			...
			break;
		default:
			S9xTracef( "Print f: Color depth %d not supported!\n");
			S9xExit ();
			break;
	}
	S9xTracef( "Print f: Found %d bit display\n", ourvideo.bitdepth);
...
#ifdef USE_XF86VIDMODE
   {
	...
	if (orig_mode->hdisplay == 320 && orig_mode->vdisplay == 240) {
		...
		if (mod320x240 == NULL) {
			S9xTracef( "Print f: No 320x240 mode available!\n");
			S9xExit ();
		}
