...
void S9xInitDisplay (int, char **)
{
    ...
    {
	S9xTracef( "STDERR: Failed to connect to X server.\n");
	exit (1);
    }
    ...
    if (!count)
    {
	S9xTracef( "STDERR: Your X Window System server is unwell!\n");
	exit (1);
    }
...
void SetupImage ()
{
    ...
    if (!GUI.image)
    {
	S9xTracef( "STDERR: XShmCreateImage failed, switching to XPutImage\n");
	...
	if (GUI.sm_info.shmid < 0)
	{
	    S9xTracef( "STDERR: shmget failed, switching to XPutImage\n");
	    XDestroyImage (GUI.image);
	    GUI.use_shared_memory = 0;
	}
	else
	{
	    ...
	    if (!GUI.image->data)
	    {
		S9xTracef( "STDERR: shmat failed, switching to XPutImage\n");
		XDestroyImage (GUI.image);
		shmctl (GUI.sm_info.shmid, IPC_RMID, 0);
		GUI.use_shared_memory = 0;
	    }
		...
		if (!GUI.use_shared_memory)
		{
		    S9xTracef( "STDERR: XShmAttach failed, switching to XPutImage\n");
		    XDestroyImage (GUI.image);
		    shmdt (GUI.sm_info.shmaddr);
		    shmctl (GUI.sm_info.shmid, IPC_RMID, 0);
		}
