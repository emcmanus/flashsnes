...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Looking up server's hostname (%s) @%ld\n", NetPlay.ServerHostName, S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Creating socket @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Trying to connect to server @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Sending 'HELLO' message @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Waiting for 'WELCOME' reply from server @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Got 'WELCOME' reply @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Sending 'READY' to server @%ld...\n", S9xGetMilliTime () - START);
#endif
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Pause - %s @%ld\n", paused ? "YES" : "NO", S9xGetMilliTime () - START);
#endif
    uint8 pause [7];
    uint8 *ptr = pause;
    *ptr++ = NP_CLNT_MAGIC;
    ...
    if (S9xNPConnect ())
    {
        ...
        while (NetPlay.Connected)
        {
            if (S9xNPWaitForHeartBeat ())
            {
                ...
                {
#ifdef NP_DEBUG
                    S9xTracef( "Print f: CLIENT: ReleaseSemaphore failed - already hit max count (%d) %ld\n", NP_JOYPAD_HIST_SIZE, S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Client thread exiting @%ld\n", S9xGetMilliTime () - START);
#endif
}
#endif
...
bool8 S9xNPCheckForHeartBeat (uint32 time_msec)
{
    ...
    S9xTracef( "Print f: CLIENT: S9xCheckForHeartBeat %s @%ld\n", (i?"successful":"still waiting"), S9xGetMilliTime () - START);
#endif
...
bool8 S9xNPWaitForHeartBeat ()
{
    ...
    while (S9xNPGetData (NetPlay.Socket, header, 3 + 4))
    {
        ...
        if ((header [2] & 0x1f) == NP_SERV_JOYPAD)
        {
            ...
            {
                //S9xTracef( "Print f: (%d)", (NetPlay.JoypadWriteInd - NetPlay.JoypadReadInd) % NP_JOYPAD_HIST_SIZE); fflush (stdout);
            }
//S9xTracef( "Print f: CLIENT: HB: @%d\n", S9xGetMilliTime () - START);
            return (TRUE);
        }
        else
        {
	    ...
	    case NP_SERV_RESET:
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: RESET received @%ld\n", S9xGetMilliTime () - START);
#endif
	    ...
	    case NP_SERV_PAUSE:
...
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: LOAD_ROM received @%ld\n", S9xGetMilliTime () - START);
#endif
                S9xNPDiscardHeartbeats ();
...
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: ROM_IMAGE received @%ld\n", S9xGetMilliTime () - START);
#endif
                S9xNPDiscardHeartbeats ();
...
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: SRAM_DATA received @%ld\n", S9xGetMilliTime () - START);
#endif
                S9xNPDiscardHeartbeats ();
...
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: FREEZE_FILE received @%ld\n", S9xGetMilliTime () - START);
#endif
                S9xNPDiscardHeartbeats ();
            ...
            default:
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: UNKNOWN received @%ld\n", S9xGetMilliTime () - START);
#endif
                S9xNPDisconnect ();
                return (FALSE);
	    }
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Asking GUI thread to open ROM load dialog...\n");
#endif

    PostMessage (GUI.hWnd, WM_USER + 3, (uint32) rom_name, (uint32) rom_name);

#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Waiting for reply from GUI thread...\n");
#endif

    WaitForSingleObject (NetPlay.ReplyEvent, INFINITE);

#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Got reply from GUI thread (%d)\n", NetPlay.Answer);
#endif
#endif
...
bool8 S9xNPGetROMImage (uint32 len)
{
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Hi-ROM: %s, Size: %04x\n", rom_info [0] ? "Y" : "N", CalculatedSize);
#endif
    ...
    // Load up ROM image
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Receiving ROM image @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Receiving ROM filename @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Receiving freeze file information @%ld...\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: Receiving freeze file @%ld...\n", S9xGetMilliTime () - START);
#endif
...
void S9xNPStepJoypadHistory ()
{
    ...
    {
#ifdef NP_DEBUG
        S9xTracef( "Print f: *** CLIENT: S9xNPStepJoypadHistory NOT OK@%ld\n", S9xGetMilliTime () - START);
#endif
    }
}
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: ResetJoyReadPos @%ld\n", S9xGetMilliTime () - START); fflush (stdout);
#endif
...
bool8 S9xNPSendData (int socket, const uint8 *data, int length)
{
	...
	if (sent < 0)
	{
		...
		|| errno == EWOULDBLOCK
            ...
            {
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: EINTR, EAGAIN or EWOULDBLOCK while sending data @%ld\n", S9xGetMilliTime () - START);
#endif
		continue;
            }
	    return (FALSE);
...
bool8 S9xNPGetData (int socket, uint8 *data, int length)
{
    ...
    do
    {
		...
		|| errno == EWOULDBLOCK
            ...
            {
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: EINTR, EAGAIN or EWOULDBLOCK while receiving data @%ld\n", S9xGetMilliTime () - START);
#endif
		continue;
            }
#ifdef WSAEMSGSIZE
            ...
            else
            {
                got = num_bytes;
#ifdef NP_DEBUG
                S9xTracef( "Print f: CLIENT: WSAEMSGSIZE, actual bytes %d while receiving data @%ld\n", got, S9xGetMilliTime () - START);
#endif
            }
#else
            return (FALSE);
...
bool8 S9xNPInitialise ()
{
    ...
    if (!initialised)
    {
        ...
        S9xTracef( "Print f: CLIENT/SERVER: Initialising WinSock @%ld\n", S9xGetMilliTime () - START);
#endif
...
#ifdef NP_DEBUG
    S9xTracef( "Print f: CLIENT: DiscardHeartbeats @%ld, finished @", S9xGetMilliTime () - START);
    fflush (stdout);
#endif
...
{
#ifdef NP_DEBUG
    S9xTracef( "Print f: NPSetAction: %s, forced = %d %ld\n", action, force, S9xGetMilliTime () - START);
#endif
