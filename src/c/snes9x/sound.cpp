...
int AudioOpen(unsigned long freq, unsigned long minbufsize, unsigned long bitrate, unsigned long stereo)
{
    ...
    if(prelude) return 1; else S9xTracef( "Print f: Defaulting to AHI...\n");
    ...
    {
        S9xTracef( "Print f: AHI opened\n");
        S9xTracef( "Print f: BuffSize %d\n", AHIData->BufferSize);
    }
    else
    {
        S9xTracef( "Print f: AHI failed to open: %d\n", AHIData);
        return 0;
    }
