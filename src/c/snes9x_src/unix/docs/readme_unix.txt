...
2. Requirements
 ...
 4. Available Snes9X command line options
      ...
      See section 11 to edit the keyboard/joystick mapping.
 ...
 9. How to play BS games
      ...
      2) Rename the BS-X ROM to 'BS-X.bin'.
      3) Put BS-X.bin in the proper directory:
         the same directory as .srm files.
      4) Launch BS-X.bin as you usually play SNES games.
 ...
 10. How to play multiple cartridge systems

      Sufami Turbo:

      1) Prepare the Sufami Turbo base cartridge (BIOS) ROM image.
         Make sure the file is uncompressed and doesn't have a header.
      2) Rename the BIOS ROM to 'STBIOS.bin'.
      3) Put STBIOS.bin in the proper directory:
         the same directory as .srm files.
      4) use -multi, -carta and -cartb options to play games.
         -multi
         BIOS only
      ...
      Note that the base cartridge can initialize SRAM for only the game
      in slot A, so you have to set every games in slot A and launch
      at least once.
      If no games are set in the slots, only the base cartridge runs.
      If you have a combined image, split it to each games
      and the base cartridge. NSRT will help you.

      Same Game + add-on cartridge:
      1) use -multi option as follows:
         -multi -carta samegame.smc -cartb addon.smc


 11. Configuring settings and key mapping
 ...
 12. Building Snes9X from source
